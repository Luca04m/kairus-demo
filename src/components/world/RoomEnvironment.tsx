  ...
  return (
    <div className="absolute inset-x-0 top-0 pointer-events-none" style={{ zIndex: 1 }}>
      <svg width={roomWidth} height={WALL_HEIGHT + 20} style={{ imageRendering: 'pixelated' }}>
        ...
        {windows.map((win, i) => (
          <g key={`win-${i}`}>
            <rect x={win.x - 4} y={win.y - 4} width={WINDOW_W + 8} height={WINDOW_H + 8} fill={wallDark} rx="2" />
            <defs>
            ...
            <rect x={win.x} y={win.y} width={WINDOW_W} height={WINDOW_H} fill={`url(#win-grad-${domain}-${i})`} rx="1" />
            <line x1={win.x + WINDOW_W / 2} y1={win.y} x2={win.x + WINDOW_W / 2} y2={win.y + WINDOW_H} stroke={wallDark} strokeWidth="2" />
            <line x1={win.x} y1={win.y + WINDOW_H / 2} x2={win.x + WINDOW_W} y2={win.y + WINDOW_H / 2} stroke={wallDark} strokeWidth="2" />
            <rect x={win.x - 6} y={win.y + WINDOW_H + 2} width={WINDOW_W + 12} height={5} fill={trimColor} rx="1" />
            <rect x={win.x + 4} y={win.y + WINDOW_H + 8} width={WINDOW_W - 8} height={30} fill={d.tileColor} opacity="0.04" rx="4" />
          </g>
        ))}
      ...
      <div className="absolute left-0 top-0 w-3 pointer-events-none" style={{ height: '100%', background: `linear-gradient(to right, ${wallDark}44, transparent)` }} />
      <div className="absolute right-0 top-0 w-3 pointer-events-none" style={{ height: '100%', background: `linear-gradient(to left, ${wallDark}44, transparent)` }} />
    </div>
  );
}
...
function WallClock({ color }: { color: string }) {
  ...
  return (
    <svg width="28" height="28" viewBox="0 0 28 28" style={{ imageRendering: 'pixelated' }}>
      ...
      <line x1="14" y1="14" x2={14 + 6 * Math.cos(hourAngle * Math.PI / 180)} y2={14 + 6 * Math.sin(hourAngle * Math.PI / 180)} stroke={color} strokeWidth="2" strokeLinecap="round" />
      <line x1="14" y1="14" x2={14 + 8 * Math.cos(minuteAngle * Math.PI / 180)} y2={14 + 8 * Math.sin(minuteAngle * Math.PI / 180)} stroke="#ccc" strokeWidth="1.5" strokeLinecap="round" />
      <circle cx="14" cy="14" r="1.5" fill={color} />
    </svg>
  );
}

function DomainPlaque({ domain }: { domain: DepartmentId }) {
  const domains = useDomains();
  const d = domains[domain];
  return (
    <div
